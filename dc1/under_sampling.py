...
for i in range(len(unique)):
    # If the class_count below mean:
    if round(counts.mean()) - counts[i] >= 0:
        # Add the those to final:
        for idx in np.where(y == i)[0]:
            augmented_data.append(X[idx])
            augmented_target.append(y[idx])
        # Calculate the amount needed to augment
        size = round(counts.mean() - counts[i])
        # If size needed to augement larger than available, just take the available
        if size > len(np.where(y==i)[0]):
            size = len(np.where(y==i)[0])
        indexes.append(
            np.random.choice(
                np.where(y == i)[0],
                size=size,
        ...
        size = round(counts.mean())
        remain_indexes.append(
            np.random.choice(
                np.where(y == i)[0],
                size=size,
                replace=False,
...
remain_indexes = np.concatenate(remain_indexes)
...
for i in range(0, len(remain_indexes)):
    augmented_data.append(X[remain_indexes[i]])
    augmented_target.append(y[remain_indexes[i]])
...
aug_X_train = np.array(augmented_data_subset)
aug_Y_train = np.array(augmented_target_subset)

# Save arrays to npy file
cwd = os.getcwd()
if path.exists(path.join(cwd + "./undersampling_data/")):
    print("Data directory exists, files may be overwritten!")
else:
    os.mkdir(path.join(cwd, "./undersampling_data/"))
    np.save("./undersampling_data/X_train.npy", aug_X_train)
    np.save("./undersampling_data/Y_train.npy", aug_Y_train)